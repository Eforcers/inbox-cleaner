...
def reset_counter(name, period=datetime.now(), period_types=[PeriodType.ALL], namespace='default', batch_size=None):
    count = load_and_get_count(name=name, namespace=namespace,  period=period)
    if not count:
        count = 0
    load_and_decrement_counter(name, period, period_types, namespace, count, batch_size)
